...
- OP-001 shall be clearly discoverable from Mission Framework's relevant navigation/onboarding surfaces (not only from the book). **Addressed 2026-09-16:** the top-level `README.md` "Foundational Documents" index (the repository's actual onboarding/navigation surface, which already listed the sibling Framework Findings Process) did not list OP-001 — a small, obvious navigation defect, not an architectural gap. Fixed by adding one entry in the same list, same format, linking to `docs/operational/OP-001-Mission-Operational-Preamble.md`. No redesign performed.
